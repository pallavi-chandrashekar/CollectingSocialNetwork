...
def get_users(twitter, screen_names):
    ...
    [6253282, 783214]
    """
    users_list = robust_request(twitter, 'users/lookup', {'screen_name': screen_names})
    return users_list
...
def get_friends(twitter, screen_name):
    ...
    list_of_friends = robust_request(twitter,'friends/ids',{'screen_name':screen_name},max_tries = 5)

    list = [i for i in list_of_friends]
    list.sort()
...
def count_friends(users):
    ...
    no_of_friends = Counter()
    friends = []
    for i in users:
        friends.append(i['friends'])
    for f in friends:
        no_of_friends.update(f)
...
def friend_overlap(users):
    ...
    for i in range(0,len(users)):
        for j in range(0,len(users)):
            if (i < j):
                user1 = users[i]['screen_name']
                user2 = users[j]['screen_name']
                check = [m for m in users[i]['friends'] if m in users[j]['friends']]
                overlap_list.append((user1, user2, len(check)))
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    clinton_friends = set(users[2]['friends'])
    trump_friends = set(users[3]['friends'])
    common_friend = clinton_friends.intersection(trump_friends)
    common_friend_list = list(common_friend)
    print(common_friend_list)
    friend_name = robust_request(twitter,'users/lookup',{'user_id':common_friend_list[0]},max_tries = 5)

    friend = [num['screen_name'] for num in friend_name]
...
def create_graph(users, friend_counts):
    ...
    Returns:
      A networkx Graph
    """
    graph_created = nx.Graph()

    for user in users:
        for f in user['friends']:
            if friend_counts[f] > 1:
                graph_created.add_edge(user['screen_name'],f)
...
def draw_network(graph, users, filename):
    ...
    try:
        ...
        label = {num: num if num in candidates else '' for num in graph.nodes()}

        figure = plot.figure()
        nx.draw_networkx(graph, pos=nx.spring_layout(graph), labels = label,alpha = 0.5, width = 0.1, node_size = 50, node_color = 'black', linewidths = 2)
        plot.axis('off')
